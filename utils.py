...
class SalEval(object):
    ...
    @torch.no_grad()
    def addBatch(self, predict, target):
        assert len(predict.shape) == 3 and len(target.shape) == 3
        for t in range(self.nthresh):
            bi_res = predict > self.thresh[t]
            intersection = torch.sum(torch.sum(bi_res & target, dim=1), dim=1).float()
            all_gth = torch.sum(torch.sum(target, dim=1), dim=1).float()
        ...
        self.mae += torch.sum(torch.abs(target.float() - predict)) / (target.shape[1] * target.shape[2])
        self.num += target.shape[0]
